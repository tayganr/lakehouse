...
Before we can test that our pipeline is able to successfully isolate and copy changes from a particular time period, we must perform some changes to our source table (e.g. UPDATE existing rows, INSERT new rows).
